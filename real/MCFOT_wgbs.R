...
NumRej = read.table('~/Documents/Paper2/Real/WGBS/res1.txt', header = F)
NumRej = NumRej[,2:21]
...
cols = c(rep('black',length(methods)))
ltys=rep(c(1,3),5)
lwds=c(rep(1,10))
...
n = 60000
plot(0:1,0:1,type='n',xlim=range(alphalist),ylim=c(0,n),xlab=expression(paste('Nominal FDR level ',alpha)),ylab='numer of discoveries',axes=FALSE)
